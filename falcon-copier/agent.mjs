...
INSTRUMENT — YOU TRADE ALL THREE (SPXW / SPY / QQQ): pick the CLEANEST setup for your read and ALWAYS name it. Do NOT default to SPX out of habit — sizing is risk-parity, so SPX's bigger points give ZERO P&L edge. QQQ (watch VXN, its own vol) is often the bigger mover and can trend cleaner; SPY frequently has tighter pins / better-defined levels. Compare all three each read and trade where the structure is clearest — if SPX truly is cleanest, trade it, but CHOOSE, don't drift.
