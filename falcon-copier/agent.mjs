...
function assembleInstrument(sym, etStr) {
  ...
  return {
    ...
    velocity: { note: 'RATE-OF-CHANGE of the key structure over the last ~6 min (M/min) + ACCEL vs the prior 6 min — read growth AND decay as sharply as the levels. net_gamma_M_per_min: + = gamma BUILDING (toward pin / mean-revert); − = DECOMPRESSING (toward accelerant / trend). dom_neg_M_per_min: − = overhead wall GROWING (resistance building); + = wall SHRINKING / giving way (escalator = trend continuation). A wall breaking or a regime flipping shows in VELOCITY before the level does — use it to get ahead of the move, and "accelerating" = conviction behind the shift.', net_gamma_M_per_min: vNet.rate, net_gamma_accel: vNet.accel, dom_neg_M_per_min: vDn.rate, dom_neg_accel: vDn.accel, king_gex_M_per_min: vKing.rate, king_gex_accel: vKing.accel },
...
- NEW perception layers (each carries its own in-line usage note — read it): higher_timeframe_trend = the DAILY up/down anchor; check your intraday dominant_trend against it and treat a counter-daily intraday move as a PULLBACK to fade WITH the daily, not a reversal to chase (raise the bar to fight the daily). opening = today's OPEN (bull/bear line) + opening range; an ORH/ORL break that ACCEPTS is a real commit, a poke-and-snap-back is noise. session = minutes_to_close + PHASE; respect 0DTE theta as time runs out and RAISE the bar / prefer fade-or-stand-aside in the LUNCH_LULL (11:30-13:30) low-signal window — that is exactly where the 8/13 whipsaw shorts were greenlit. vanna_flow = the VIX->direction dealer transmission; do NOT short into a rising positive-vanna bid (falling VIX = mechanical dealer buying). vol_dynamics = the IV/vol TRAJECTORY (CRUSHING = pin-confirm so fade / don't chase a breakout; EXPANDING = real move so hold the runner) + IV-vs-REALIZED (IV>>RV = the theta trap, stand aside not chase) + front-kink (inverted = size down). USE THEM TOGETHER: the strongest reads stack GEX structure + a key_level + the daily trend + a supportive session phase + vanna/VIX all pointing the same way; when they CONFLICT, size down or stand aside. This is how you tell a dip-in-a-trend from a reversal — the call the 8/13 shorts got wrong.
- RISK STATE (your OWN book — shown above the data each tick) = you looking at YOURSELF. After a loss or while in DRAWDOWN, get DEFENSIVE: raise the bar, prefer the fade/stand-aside, and do NOT revenge-trade to win it back (that IS the pin-chasing spiral). The open-tranche cap is 4 COMBINED across BOTH postures (NOT 4 each) — when it says no slots left, an add WILL be refused, so don't plan one. If both postures collapse onto the SAME instrument+direction you're running a DOUBLED bet — only at genuine top conviction. And if a SYSTEM OVERRIDE is shown (an add/entry you signaled got refused), your journal may claim a position you do NOT hold — RECONCILE it before reasoning from it.
...
- PREMIUM-TRAILING TAKE (automatic — the OPTION-value mirror of the theta stop): once an option has spiked ≥ +60%, if it gives back ≥ 30% of its premium from that peak, the system BANKS it — capturing the option's high before theta / IV-crush / a reversal erodes it. So a fast option spike is protected even when your price target is far (e.g. a runner). You do not set this; it runs alongside your price stop/target. It means a sharp IV/gamma pop mid-move gets harvested rather than round-tripped.
...
const sysBlocks = (s) => Array.isArray(s) ? s : [{ type: 'text', text: s, cache_control: { type: 'ephemeral' } }];   // string → one cached block (reflect/distill); array → used as-is, so the 2-stage shares ONE identical [doctrine, state] cached prefix across both calls (tools→system→messages is the cache key; forcing a tool differs but doesn't invalidate the cached tools/system)
async function claude(system, content, tool, maxTok = 1800) {
  ...
  const r = await fetch('https://api.anthropic.com/v1/messages', { method: 'POST', headers: { 'x-api-key': KEY, 'anthropic-version': '2023-06-01', 'content-type': 'application/json' }, body: JSON.stringify({ model: MODEL, max_tokens: maxTok, system: sysBlocks(system), messages: [{ role: 'user', content }], tools: [tool], tool_choice: { type: 'tool', name: tool.name } }) });
...
async function reason(system, content) {
  const r = await fetch('https://api.anthropic.com/v1/messages', { method: 'POST', headers: { 'x-api-key': KEY, 'anthropic-version': '2023-06-01', 'content-type': 'application/json' }, body: JSON.stringify({ model: MODEL, max_tokens: 2500, system: sysBlocks(system), messages: [{ role: 'user', content }], tools: [TOOL], tool_choice: { type: 'none' } }) });
  const j = await r.json(); if (r.status !== 200) throw new Error(`API ${r.status}: ${JSON.stringify(j.error || j).slice(0, 200)}`);
...
async function manage(book, mode, dec, instruments, et, trend) {
  ...
  for (const pos of [...held]) {                                          // copy — closePosition() splices b.positions
    ...
    else if (pos.peak_ret_pct != null && pos.peak_ret_pct >= PREM_TAKE_MIN && pos.peak_premium && pos.live_premium != null && (pos.peak_premium - pos.live_premium) / pos.peak_premium >= PREM_TRAIL_GIVEBACK) why = `premium take (+${pos.live_ret_pct}% off a +${pos.peak_ret_pct}% peak)`;   // OPTION-value trailing take: the option spiked then gave back — bank it before theta/IV-crush erodes the rest (mirror of the theta stop)
  ...
  let _addReason = null; const veto = r => { (book._vetoes ||= []).push(mode + ': ' + r); };   // record when the system OVERRODE a signaled open/add → surfaced to the agent next tick (kills self-model desync)
  const _dayRealized = ['conservative', 'aggressive'].reduce((a, m) => a + (book[m]?.closed || []).reduce((s, c) => s + (c.pnl_usd || 0), 0), 0);
  if (_dayRealized <= DAILY_LOSS_LIMIT) { veto('NEW RISK BLOCKED — daily circuit breaker breached'); return; }   // CIRCUIT BREAKER: combined realized loss breached the daily limit → no new risk
  const _totalOpen = (book.conservative?.positions?.length || 0) + (book.aggressive?.positions?.length || 0);
  if (_totalOpen >= MAX_TOTAL) { veto(`ADD/ENTRY REFUSED — ${MAX_TOTAL}-tranche COMBINED cap already full across both postures`); return; }   // GLOBAL correlation cap across both postures + instruments
  const _nowMin = (typeof et === 'string' && et.length >= 5) ? (+et.slice(0, 2)) * 60 + (+et.slice(3, 5)) : null;
  if (_cooldown[`${inst}_${decDir}`] != null && _nowMin != null && _nowMin - _cooldown[`${inst}_${decDir}`] < COOLDOWN_MIN) { veto(`ENTRY REFUSED — re-entry cooldown active (${inst} ${decDir}, just stopped out)`); return; }   // RE-ENTRY COOLDOWN: just stopped out this instrument+dir
  ...
  if (openCount === 0) allow = conv >= ENTRY_BAR && reached;              // first tranche: decisive + (resting entry) reached
    ...
    isAdd = true;
    if (!allow) _addReason = dec.scale_in !== true ? 'you did not set scale_in=true' : !trendOK ? 'add not aligned with your dominant_trend' : !allGreen ? 'stack is RED — never add to losers' : conv < ENTRY_BAR ? 'conviction below the add bar' : openCount >= MAX_TRANCHES ? 'posture already at MAX_TRANCHES' : et >= ADD_CUTOFF_ET ? 'past the add-cutoff time' : 'risk guard';
  }
  if (!allow) { if (isAdd && _addReason) veto('ADD REFUSED — ' + _addReason); return; }
...
// SELF-RISK HUD — the agent's OWN book/risk state: realized P&L, drawdown, the REAL combined cap, netted exposure, cooldowns, and last-tick SYSTEM OVERRIDES. All compute-from-book (no external data). Fixes the self-model desync + the cap mis-tell the audit flagged.
function riskHUD(book, nowMin, lastVetoes, dd) {
  const P = m => (book[m]?.closed || []).reduce((s, c) => s + (c.pnl_usd || 0), 0);
  const rc = P('conservative'), ra = P('aggressive'), tot = rc + ra;
  const openC = book.conservative?.positions?.length || 0, openA = book.aggressive?.positions?.length || 0, openTot = openC + openA, room = MAX_TOTAL - openTot;
  const nClosed = (book.conservative?.closed?.length || 0) + (book.aggressive?.closed?.length || 0);
  const exp = {}; for (const m of ['conservative', 'aggressive']) for (const p of (book[m]?.positions || [])) { const k = `${p.instrument} ${p.dir}`; exp[k] = (exp[k] || 0) + 1; }
  const expStr = Object.keys(exp).length ? Object.entries(exp).map(([k, n]) => `${n}× ${k}`).join(', ') : 'flat';
  const concentrated = openTot >= 2 && Object.keys(exp).length === 1;   // both postures collapsed onto the SAME instrument+direction = a doubled bet
  const allClosed = ['conservative', 'aggressive'].flatMap(m => (book[m]?.closed || []).map(c => ({ ...c, m }))).sort((a, b) => (a.exitET || '').localeCompare(b.exitET || ''));
  const lastEx = allClosed.slice(-3).map(c => `${c.exitET || '?'} ${c.m[0]}:${c.dir} ${c.pnl_usd >= 0 ? '+' : ''}$${c.pnl_usd} (${(c.why || '').split(/[ (]/)[0]})`).join(' · ') || 'none yet';
  const cds = Object.entries(_cooldown || {}).filter(([, t]) => nowMin != null && nowMin - t < COOLDOWN_MIN).map(([k]) => k);
  return [
    'YOUR RISK STATE (perceive it — get DEFENSIVE after losses, do NOT revenge-trade, respect the REAL cap):',
    `  • realized today: COMBINED ${tot >= 0 ? '+' : ''}$${tot} (cons ${rc >= 0 ? '+' : ''}$${rc} · agg ${ra >= 0 ? '+' : ''}$${ra}) over ${nClosed} closed${dd != null && dd < -200 ? ` · in DRAWDOWN ${dd} from today's peak → tighten up, stop pressing` : ''}`,
    `  • circuit breaker ${DAILY_LOSS_LIMIT}: ${tot > DAILY_LOSS_LIMIT ? `$${tot - DAILY_LOSS_LIMIT} of room before ALL new risk is cut for the day` : 'BREACHED — no new risk today'}`,
    `  • OPEN tranches: ${openTot}/${MAX_TOTAL} COMBINED across both postures (cons ${openC}, agg ${openA}). GLOBAL cap — SPXW/SPY/QQQ are ~ONE correlated bet — ${room <= 0 ? 'NO slots left: any ADD or new entry (EITHER posture) WILL be refused' : `${room} slot(s) left in TOTAL (this is NOT per-posture — the whole book shares 4)`}`,
    `  • exposure: ${expStr}${concentrated ? '  ⚠ BOTH postures on the SAME instrument+direction = a DOUBLED, concentrated bet. The two postures are meant to DIFFER — is this genuinely your single highest-conviction trade at 2× size?' : ''}`,
    `  • recent exits: ${lastEx}${cds.length ? `\n  • COOLDOWN active (re-entry blocked, just stopped out): ${cds.join(', ')}` : ''}`,
    ...(lastVetoes && lastVetoes.length ? [`  • ⚠ LAST TICK the SYSTEM OVERRODE you (your journal may claim an action that did NOT execute — reconcile before trusting it): ${lastVetoes.join(' | ')}`] : []),
  ].join('\n');
}
// VOL DYNAMICS — the price-blindness fix applied to VOL. Inputs (VIX, vix1d_0dte, IV30, ATM straddle) are already fetched each tick;
// buffer them + emit TRAJECTORY (IV crushing=pin-confirm vs expanding=real-move), IV-vs-REALIZED (the theta-trap gauge), VIX path, and the 0DTE front-kink.
function computeVolDynamics(hist, pricePath) {
  if (!hist || hist.length < 2) return null;
  const last = hist[hist.length - 1], first = hist[0];
  const v = hist.filter(h => h.vix1d != null), win = v.slice(-7);
  const vix1dChg = (win.length >= 2 && win[0].vix1d != null) ? +(win[win.length - 1].vix1d - win[0].vix1d).toFixed(2) : null;
  let rv = null;
  if (pricePath && pricePath.length > 5) { const r = []; for (let i = 1; i < pricePath.length; i++) r.push(Math.log(pricePath[i] / pricePath[i - 1])); const mu = r.reduce((a, b) => a + b, 0) / r.length, va = r.reduce((a, b) => a + (b - mu) ** 2, 0) / r.length; rv = +(Math.sqrt(va) * Math.sqrt(252 * 390) * 100).toFixed(1); }   // annualized realized vol %, comparable to VIX1D
  const iv = last.vix1d, ivrv = (iv != null && rv) ? +(iv / rv).toFixed(2) : null, kink = (last.vix1d != null && last.iv30 != null) ? +(last.vix1d - last.iv30).toFixed(1) : null;
  return {
    vix1d_0dte: iv, vix1d_chg_recent: vix1dChg,
    iv_trend: vix1dChg == null ? null : vix1dChg < -0.3 ? 'CRUSHING (vol collapsing → pin confirming: fade extremes, do NOT chase a breakout — it will fail)' : vix1dChg > 0.3 ? 'EXPANDING (vol rising → real move/breakout: hold the runner, levels can blow through)' : 'flat',
    realized_vol_30m: rv, iv_vs_realized: ivrv, iv_rv_read: ivrv == null ? null : ivrv > 1.5 ? 'IV >> RV: option is RICH — you are paying for a move that is NOT happening (the theta trap, the #1 measured leak); favor fade/stand-aside, not chasing' : ivrv < 0.9 ? 'IV < RV: option is CHEAP vs the actual move — a real trend is underpriced, lean to HOLD' : 'fair',
    vix_chg_since: (last.vix != null && first.vix != null) ? +(last.vix - first.vix).toFixed(2) : null,
    front_kink_vix1d_minus_iv30: kink, front_kink_read: kink == null ? null : kink > 2 ? 'INVERTED (0DTE vol >> 30d): acute/violent regime — levels blow through, size DOWN, expect TREND not pin' : 'normal',
    note: "VOL TRAJECTORY (price-blindness, for vol): IV CRUSHING intraday confirms a pin (fade); IV EXPANDING = a real move (hold). IV-vs-REALIZED is the theta-trap gauge (IV>>RV = overpaying for a move that isn't there). front_kink inverted = size down. Cross with vanna_flow + vix.tilt.",
  };
}
const LIVE = !!arg('--live', false);
async function step(et, mem) {
  const state = await assembleComplex(et, LIVE), R = state.instruments.SPXW;
  const _vx = state.uw_layers?.vix, _strd = state.instruments?.SPXW?.expected_move?.atm_straddle;   // VOL DYNAMICS: buffer the per-tick vol snapshot → emit trajectory / IV-vs-RV / front-kink (warms up over ~7 ticks)
  if (LIVE && _vx) { mem._volHist = (mem._volHist || []).concat({ et, vix: _vx.level, vix1d: _vx.vix1d_0dte, iv30: _vx.spy_iv_30d, straddle: _strd }).slice(-90); state.vol_dynamics = computeVolDynamics(mem._volHist, R.price_path_30m); }
  ...
  const _lastVetoes = mem.book._vetoes || []; mem.book._vetoes = [];   // last tick's SYSTEM OVERRIDES → surface once, then reset for this tick's manage()
  const _totReal = ['conservative', 'aggressive'].reduce((a, m) => a + (mem.book[m]?.closed || []).reduce((s, c) => s + (c.pnl_usd || 0), 0), 0);
  const _prevPeak = mem._peakRealized ?? 0, _dd = _totReal - _prevPeak; mem._peakRealized = Math.max(_prevPeak, _totReal);   // drawdown from today's peak realized
  const _nowMin = (typeof et === 'string' && et.length >= 5) ? (+et.slice(0, 2)) * 60 + (+et.slice(3, 5)) : null;
  const planNote = (m) => {
    const ps = mem.book[m]?.positions || []; if (!ps.length) return `${m}: flat`;
    const dir = ps[0].dir, lines = ps.map((o, i) => `#${i + 1} ${o.instrument} ${o.strike}${o.cp} @${o.entryPx} ${o.live_ret_pct != null ? (o.live_ret_pct >= 0 ? '+' : '') + o.live_ret_pct + '%' : 'new'} stop→${o.stop_level ?? '?'} held ${o.entryET && _nowMin != null ? (_nowMin - ((+o.entryET.slice(0, 2)) * 60 + (+o.entryET.slice(3, 5)))) + 'm' : '?'}`).join(' | ');
    return `${m}: HOLDING ${ps.length} ${dir} tranche(s) [${lines}] · tgt ${ps[0].target_level ?? '?'}. MANAGE the stack: repeat "${dir}" to HOLD as-is; set scale_in=true to ADD a $${NOTIONAL_PER_POSITION / 1000}k tranche when the trend CONFIRMS at a new structural level (price ACCEPTED a node / king rolled ${dir === 'long' ? 'up' : 'down'}) — your call, but adds are subject to the ${MAX_TOTAL}-tranche COMBINED cap across BOTH postures (see RISK STATE), all-green, trend-aligned, before ${ADD_CUTOFF_ET} (system refuses a bad one); RAISE stop_level (STRUCTURAL — under the GEX support / king / last higher-${dir === 'long' ? 'low' : 'high'}) to trail EVERY tranche up; stand_aside/reverse (conv ≥${EXIT_BAR}) to flatten the WHOLE stack. System auto-exits each tranche at its own stop/target, a hard ${MAX_OPT_LOSS_PCT}% premium stop, and flattens all near close.`;
  };
  const bookNote = `${riskHUD(mem.book, _nowMin, _lastVetoes, _dd)}\n\nYOUR OPEN POSITIONS & PLANS (manage them — don't re-decide from scratch):\n  ${planNote('conservative')}\n  ${planNote('aggressive')}`;
...
if (arg('--state', false)) {   // debug: eyeball the NEW per-instrument state fields (htf trend / opening / vanna / session) from the on-disk frames
  ...
  try {   // risk HUD render check — real 8/13 book (P&L/exits) + a synthetic cap-full book (corrected cap message + concentration warning)
    const sf = path.join(FC, 'agent_state_2026-08-13.json');
    if (fs.existsSync(sf)) console.log('\n═══ RISK HUD · real 8/13 book ═══\n' + riskHUD(JSON.parse(fs.readFileSync(sf, 'utf8')).book, 960, ['aggressive: ADD REFUSED — 4-tranche COMBINED cap already full'], -1832));
    const mock = { conservative: { positions: [{ instrument: 'SPXW', dir: 'long', entryET: '10:01' }, { instrument: 'SPXW', dir: 'long', entryET: '10:05' }], closed: [] }, aggressive: { positions: [{ instrument: 'SPXW', dir: 'long', entryET: '09:36' }, { instrument: 'SPXW', dir: 'long', entryET: '09:52' }], closed: [] } };
    console.log('\n═══ RISK HUD · synthetic 4-open (cap-full + concentration) ═══\n' + riskHUD(mock, 660, [], 0));
  } catch (e) { console.log('hud err', e.message); }
  try {   // vol-dynamics render check on a synthetic warming history (IV crushing + IV>>RV theta-trap)
    const hist = []; for (let i = 0; i < 8; i++) hist.push({ et: '11:0' + i, vix: 15 - i * 0.1, vix1d: 22 - i * 0.5, iv30: 16, straddle: 30 - i });
    const flatPath = Array.from({ length: 20 }, (_, i) => 7800 + Math.sin(i) * 2);
    console.log('\n═══ VOL DYNAMICS · synthetic (IV crushing, IV>>RV) ═══\n' + JSON.stringify(computeVolDynamics(hist, flatPath), null, 1));
  } catch (e) { console.log('vol err', e.message); }
  process.exit(0);
}
