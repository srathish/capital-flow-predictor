...
function FlowAggregatePanel() {
  ...
  async function go() {
    ...
    setError(null);
    try {
      const res = await api.flowAggregate(ticker.trim().toUpperCase(), days);
      setData(res);
    } catch (e) {
      setError(e instanceof Error ? e.message : "load failed");
    } finally {
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-base font-medium">Flow aggregate · all UW alerts over window</h3>
        <span className="text-xs text-muted-foreground">
          Bull/bear lean · top strikes by $ premium · largest single tickets
        </span>
      </div>
      <div className="mb-3 flex gap-2">
        <input
      ...
      {data && (
        <div className="space-y-3 text-sm">
          ...
          {data.n_alerts === 0 && (
            <p className="text-xs text-muted-foreground">
              No UW flow alerts for {data.ticker} in the last {data.days}d. UW only retains a
              rolling window — try a smaller window, or ingest fresh data with{" "}
